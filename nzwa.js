...
/*************Auto respon************/
   ...
   if (budy.match('Hai')){costum('Hai juga saya bot ketik #help untuk melihat menu', text, tescuk, menuc)
   }
   if(budy.match('Bot')){
...
❏─────「 Note 」────`, text, tescuk, menuc)}
			...
			switch(command) {
                     ...
                     break
                                case 'educationmenu':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        await cost(education(prefix, botName, ownerName, _registered, time), text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { mimetype: "image/jpeg", "caption": `*Hai ${pushname}*`, "jpegThumbnail": fs.readFileSync('./temp/image/tahta.jpeg') } } } } )
                                        break
                                case 'funmenu':
                                        if (!isRegister) return reply(mess.only.daftarB)
...
//--Info bot
  ...
  ‣ *The bot is active on* : ${getruntime}
