...
nzwa.on('message-update', async (geps) => {
	...
	try {
		...
		let sender = geps.key.fromMe ? nzwa.user.jid : geps.key.remoteJid.endsWith('@g.us') ? geps.participant : geps.key.remoteJid
		...
		if (messageStubType == 'REVOKE') {
			...
			const conts = geps.key.fromMe ? nzwa.user.jid : nzwa.contacts[sender] || { notify: jid.replace(/@.+/, '') }
			const pushname = geps.key.fromMe ? nzwa.user.name : conts.notify || conts.vname || conts.name || '-'
			const opt4tag = {
...
• Pesan: ${body ? body : '-'}
`
				nzwa.sendMessage(from, strConversation, MessageType.text, selepbot72)
				...
				const savedFilename = await nzwa.downloadAndSaveMediaMessage(int.data, `./temp/sticker/${filename}`);
				const strConversation = `「 *ANTI-DELETE* 」
...
`
				...
				nzwa.sendMessage(from, strConversation, MessageType.text, opt4tag)
				nzwa.sendMessage(from, buff, MessageType.sticker, pingbro23)
				// console.log(stdout)
				...
				const filename = `${sender.replace('@s.whatsapp.net', '')}-${moment().unix()}`
				const savedFilename = await nzwa.downloadAndSaveMediaMessage(int.data, `./temp/image/${filename}`);
				const buff = fs.readFileSync(savedFilename)
				const strConversation = `「 *ANTI-DELETE* 」
...
• Pesan: ${body ? body : '-'}\`\`\`
`
				nzwa.sendMessage(from, buff, MessageType.image, { contextInfo: { mentionedJid: [sender] }, caption: strConversation })
				fs.unlinkSync(savedFilename)
			}
		}
	} catch (e) {
...
case 'antidelete':
					...
					if (argz[1] == 'aktif') {
						...
						if (argz.length === 2 || argz[2].startsWith('0')) return reply(`Masukan nomer diawali dengan 62! contoh 62859289xxxxx`)
